...
public class CapturedTypeConstructor(
        ...
        // This property is a kind of hack: we should treat differently captured types generated by inference and by member scope
        // of types with projection.
        // Mainly it's needed for approximation phase of captured types (except return types):
        // 1. We want to approximate such types up to their projections when building resulting descriptor from inference
        // 2. We do not want to approximate such types when they are met in member scope to preserve type safety:
        // MutableCollection<out C>.addAll should not have signature `fun addAll(elements: Collection<out C>)`,
        // it should be `fun addAll(elements: Collection<Captured(out C)>)`, so it's not possible to put anything but List<Nothing>
        public val isFromInference: Boolean
): TypeConstructor {
    init {
...
public class CapturedType(
        private val typeProjection: TypeProjection, isFromInference: Boolean
    ...
    private val delegateType = run {
        ...
        KotlinTypeImpl.create(Annotations.EMPTY, CapturedTypeConstructor(typeProjection, isFromInference), false, listOf(), scope)
    }
...
fun createCapturedType(typeProjection: TypeProjection, isFromInference: Boolean): KotlinType =
        CapturedType(typeProjection, isFromInference)
...
public fun TypeSubstitution.wrapWithCapturingSubstitution(): TypeSubstitution {
    //return this
            return if (this is IndexedParametersSubstitution)
                IndexedParametersSubstitution(
                        this.parameters,
                        this.arguments.zip(this.parameters).map {
                            it.first.createCapturedIfNeeded(it.second) as TypeProjection
                        }.toTypedArray(),
...
private fun TypeProjection?.createCapturedIfNeeded(typeParameterDescriptor: TypeParameterDescriptor?): TypeProjection? {
    if (this == null || typeParameterDescriptor == null || projectionKind == Variance.INVARIANT) return this
    ...
    if (typeParameterDescriptor.variance == projectionKind) {
        ...
        if (isStarProjection) return TypeProjectionImpl(object : DelegatingType() {
            override fun getDelegate() = this@createCapturedIfNeeded.type
        })
    }

    return TypeProjectionImpl(createCapturedType(this, isFromInference = false))
}

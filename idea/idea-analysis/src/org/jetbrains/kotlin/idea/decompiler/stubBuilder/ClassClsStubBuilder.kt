...
private class ClassClsStubBuilder(
    ...
    private fun createNestedClassStub(classBody: StubElement<out PsiElement>, nestedClassId: ClassId) {
        ...
        if (classDataWithSource == null) {
            ...
            LOG.error("Could not find class data for nested class $nestedClassId of class ${nestedClassId.outerClassId}\n" +
                      "Root file: ${rootFile.canonicalPath}\n" +
                      "Children:\n ${rootFile.children.joinToString(separator = "\n") {
                          (it.canonicalPath ?: "<no path>") + "(valid: ${it.isValid})"
                      } }")
            return
        }

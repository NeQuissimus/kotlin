    ...
    override fun extractFixData(element: KtCallableReferenceExpression, diagnostic: Diagnostic): List<CallableInfo> {
        ...
        return element
                ...
                .mapNotNull {
                    ...
                    val actualTypeRef = element.typeReference
                    val receiverTypeInfo = actualTypeRef?.let { TypeInfo(it, Variance.IN_VARIANCE) } ?: TypeInfo.Empty
                    ...
                    val parameterInfos = SmartList<ParameterInfo>().apply {
                        if (actualTypeRef == null && expectedReceiverType != null) {
                            add(ParameterInfo(TypeInfo(expectedReceiverType, Variance.IN_VARIANCE)))
                        }

                        KotlinBuiltIns
                                .getParameterTypeProjectionsFromFunctionType(it)
                                .let { if (actualTypeRef != null && expectedReceiverType == null && it.isNotEmpty()) it.subList(1, it.size) else it }
